...
def read_video_list(patch_path, datapath, phase_gen='train', shuffle=True, ext='.mp4'):
    ...
    patch_list = [patch_path + f.strip('\n') for f in lines]
    f.close()

    return patch_list, videos, vidmaps, vidfixs


def read_video_list_new(patch_path, datapath, phase_gen='train', shuffle=True, ext='.mp4'):
    if phase_gen in ['train', 'val', 'test']:
        txt_path     = datapath + '/txt_correct/' + phase_gen + '.txt'
        videos_path  = datapath + '/videos/'
        # videos_path = datapath + '/videos/'
        vidmaps_path = datapath + '/maps/'
        vidfixs_path = datapath + '/fixations/maps/'
...
def preprocess_videos_SST(path, shape_r, shape_c, frames=float('inf'), mode='BGR' ,normalize=True):
...
def st_get_gaussmaps_horizontal(height, width, nb_gaussian):
    # ���������ı���
    e = height / width
    e1 = (1 - e) / 2
    e2 = e1 + e
    ...
    # ������ȷֲ���mu_x ����Ϊ���ȷֲ�
    mu_x = np.linspace(0, 1, nb_gaussian)  # �����ϵĸ�˹�ֲ����ȷֲ�
    mu_y = np.repeat(0.5, nb_gaussian)  # �����ϵĸ�˹�ֲ��������м�

    # �����׼���Ȼʹ�ø�˹�ֲ��ı�׼��
    sigma_x = np.full_like(mu_x, 0.1)  # ����ı�׼���С����֤��������
    sigma_y = np.array([e * i / (2 * nb_gaussian) for i in range(1, nb_gaussian + 1)])  # ����ı�׼����������
...
def get_guasspriors(b_s=2, shape_r=45, shape_c=80, channels = 8):
    ...
    if not os.path.exists(priormat_path):
        ...
        #ims = st_get_gaussmaps_horizontal(shape_r, shape_c, channels)
